    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "delete")
        {
            ...
            if (row > 0)
            {
                ...
                Response.Write("<script>alert('删除成功!');window.location.href='ElementInfo.aspx';</script>");
                return;
            }
            else
            {
                Response.Write("<script>alert('删除失败!');window.location.href='ElementInfo.aspx';</script>");
                return;
            }
        }
